...
resource "aws_route_table_association" "publicr11" {
  provider = aws.singapore
  subnet_id      = aws_subnet.public11.id
  
  route_table_id = aws_route_table.public1.id
}
resource "aws_route_table_association" "publicr22" {
  provider = aws.singapore
  subnet_id      = aws_subnet.public22.id
...
resource "aws_route_table_association" "privater11" {
  provider = aws.singapore
  subnet_id      = aws_subnet.private11.id
  route_table_id = aws_route_table.private1.id
}

resource "aws_route_table_association" "privater22" {
  provider = aws.singapore
  subnet_id      = aws_subnet.private22.id
  route_table_id = aws_route_table.private1.id
}
...
output "public_subnet_idd" {
  value = [aws_subnet.public11.id,aws_subnet.public22.id]
}

output "private_subnet_idd" {
  value = [aws_subnet.private11.id,aws_subnet.private22.id]
}
...
resource "aws_db_subnet_group" "mydb_subnet_group1" {
  provider = aws.singapore
  name        = "mydb_subnet_groupp"
  subnet_ids  = [aws_subnet.private11.id,aws_subnet.private22.id]
...
resource "aws_instance" "example_server11" {
  provider = aws.singapore
  ...
  connection {
    ...
    user        = "ec2-user"
    host        = self.public_ip
    private_key = file("C:/Users/HP/bangtan_si.pem")
  }
  ...
  provisioner "file" {
    ...
    <?php
     ...
     define('DB_SERVER', '${aws_db_instance.my_db1.address}');
     define('DB_USERNAME', 'admin');
     define('DB_PASSWORD', 'Prajna0312.');
     define('DB_DATABASE', 'Students');
...
output "public_ip_address_nvv"{
  value = [aws_instance.example_server11.public_dns, aws_instance.example_server11.public_ip]
